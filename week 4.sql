...
select branchname, sum(assets) as total_assets
from branch
group by branchname;
...
select distinct customername, accno 
from depositer;

select d.customername, sum(l.amount)
from depositer d, bankaccount ba, loan l
where d.accno=ba.accno and ba.branchname=l.branchname
group by d.customername;

select d.customername, ba.city
from depositer d, bankcustomer ba
where d.customername=ba.customername and ba.city="delhi"
group by d.customername;


select d.customername, ba.balance
...
from depositer d, bankaccount ba, loan l
where d.accno=ba.accno and l.branchname=ba.branchname;
 
 select branchname, count(accno)
 from bankaccount
 group by branchname;
...
where l.loannumber is null;

update branch
set branchname="SBI_Road";